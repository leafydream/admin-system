...
const Gallery = ({videos, handleShowVideo, dataSource, onChange, pagination, status, handleCheckBox, setStatus, checkedArray }) => {
    const openGallery = (record) => {
        handleShowVideo(record);
    }

    const imgsTag = videos.map((v1, indexX) => (
        v1.map((v2, indexY) => (
            <div className="gutter-box" key={ [indexX, indexY].join('-') }>
                <Card bordered={false} bodyStyle={{ padding: 0 }}>
                    <div>
                        <img onClick={() => openGallery(v2)} alt="example" width="100%" src={v2.cover[0]} />
