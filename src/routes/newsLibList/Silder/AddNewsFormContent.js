...
import { Form, Row, Col, Input, Button, Upload, Icon, Modal, Select, notification } from 'antd';
...
class AddNewsFormContent extends Component {
    ...
    this.setState({
      ...
      if(file.status === "done"){
        this.setState({
          ...
          console.log(this.state.uploadImageList);
        });
      }
    });
  };
  ...
  componentWillReceiveProps(newProps){
    const { categoryList, newsLibItem } = newProps;
    const { editorState } = this.state;
    let editState = null;

    if(newsLibItem.contentJson){
        let state = convertFromRaw(JSON.parse(newsLibItem.contentJson));
        editState = EditorState.createWithContent(state);
    }else if(newsLibItem.content){
        const blocksFromHTML = convertFromHTML(newsLibItem.content);
    ...
    let imageArr = [];
    if(newsLibItem.imgUrls){
      if(newsLibItem.imgUrls.indexOf(newsLibItem.thumbnails)==-1){
        imageArr.push({
          thumbnails: newsLibItem.thumbnails
        });
      }
      let arr  = newsLibItem.imgUrls.split(" ");
    ...
    this.setState({
      ...
      thumbImage: newsLibItem.thumbnails ? newsLibItem.thumbnails : '',
  ...
  render() {
    const { newsLibItem } = this.props;
    const { getFieldDecorator } = this.props.form;
    const { previewVisible, previewImage, fileList, uploadImageList, categoryList, editorState } = this.state;
          ...
          <Row>
            <Col span={24} key="1" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 3}} label={`资讯标题`}>
                {getFieldDecorator(`title`,{ initialValue: newsLibItem.title })(
            ...
            <Col span={24} key="2" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 3}} label={`资讯类别`}>
                {getFieldDecorator('catId',{ initialValue: newsLibItem.catId })(
            ...
            <Col span={24} key="3" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 3}} label={`资讯类型`}>
                {getFieldDecorator('type',{ initialValue: 'REPRODUCED' })(
            ...
            <Col span={24} key="4" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 3}} label={`内容类型`}>
                {getFieldDecorator('contentType',{ initialValue: newsLibItem.contentType })(
            ...
            <Col span={24} key="5" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 5}} label={`文章作者`}>
                {getFieldDecorator(`author`,{ initialValue: newsLibItem.author })(
            ...
            <Col span={24} key="6" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 5}} label={`资讯标签`}>
                {getFieldDecorator(`labels`,{ initialValue: newsLibItem.labels })(
            ...
            <Col span={24} key="7" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 5}} label={`资讯来源`}>
                {getFieldDecorator(`origin`,{ initialValue: newsLibItem.origin })(
            ...
            <Col span={24} key="8" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 5}} label={`资讯来源url`}>
                {getFieldDecorator(`originUrl`,{ initialValue: newsLibItem.originUrl })(
            ...
            <Col span={24} key="9" >
              <FormItem labelCol={{span: 2}} wrapperCol={{span: 6}} label={`导语`}>
                {getFieldDecorator(`intro`,{ initialValue: newsLibItem.intro })(
